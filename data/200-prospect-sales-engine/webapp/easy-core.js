'use strict';
const DATA_KEY='ps200-mobile-v1', SETTINGS_KEY='ps200-settings-v1', UI_KEY='ps200-easy-ui-v3';
...
const toast=m=>{const e=$('toast');if(!e)return;e.textContent=m;e.classList.add('on');clearTimeout(toast.t);toast.t=setTimeout(()=>e.classList.remove('on'),2300)};
...
function addBusinessDays(date,count){const d=new Date(date);let left=Math.max(0,count);while(left){d.setDate(d.getDate()+1);const day=d.getDay();if(day!==0&&day!==6)left--;}return d;}
function followDueDate(p){if(!p.sentAt)return null;const days=p.lastContactType==='FU1'?5:2;return addBusinessDays(new Date(p.sentAt),days)}
function isFollowDue(p){const d=followDueDate(p);return !!d&&d.getTime()<=Date.now()}
function recommendedScript(p){
  if(p.status==='REPLIED')return (p.scriptType==='CLOSE'||p.scriptType==='QUALIFY')?p.scriptType:'QUALIFY';
  if(p.status==='SENT'&&isFollowDue(p))return p.lastContactType==='FU1'?'FU2':'FU1';
  return p.scriptType||'INIT';
}
function subjectForScript(p,type){if(type==='FU1'||type==='FU2')return `Following up — ${p.prospect||p.id}`;if(type==='QUALIFY')return `Re: ${p.prospect||p.id}`;if(type==='CLOSE')return `Confirming scope — ${p.prospect||p.id}`;return defaultSubject(p)}
function effectiveBody(p){const type=recommendedScript(p);return p.message&&p.messageScript===type?p.message:scripts[type](p)}
function effectiveSubject(p){const type=recommendedScript(p);return p.subject&&p.subjectScript===type?p.subject:subjectForScript(p,type)}
...
function processed(p){return ['SENT','REPLIED','WON / PAID','SKIP'].includes(p.status)}
function badFit(p){const s=String(p.status);return s.includes('DO NOT PRIORITIZE')||s.includes('ARCHIVE')||s.includes('DEPRIORITIZE')}
function actionable(p){if(badFit(p)||p.status==='WON / PAID'||p.status==='SKIP')return false;if(p.status==='REPLIED')return true;if(p.status==='SENT')return isFollowDue(p);return true;}
function all(){return data.map(merge)}
function nextWorkable(afterId){
  const a=all();
  const replies=a.filter(p=>p.status==='REPLIED'&&!badFit(p));if(replies.length)return replies.sort((x,y)=>(x.priority||999)-(y.priority||999))[0];
  const due=a.filter(p=>p.status==='SENT'&&isFollowDue(p)&&!badFit(p));if(due.length)return due.sort((x,y)=>followDueDate(x)-followDueDate(y))[0];
  let start=afterId?Math.max(0,a.findIndex(p=>p.id===afterId)+1):0;
  for(let n=0;n<a.length;n++){const p=a[(start+n)%a.length];if(actionable(p)&&p.status!=='SENT'&&p.status!=='REPLIED')return p}
  return a.find(actionable)||a[0];
}
function ensureCurrent(){const a=all();const existing=currentId&&a.find(p=>p.id===currentId);if(!existing||!actionable(existing))currentId=(nextWorkable(currentId)||a[0]).id;}
function counters(){const a=all();return{sent:a.filter(p=>p.status==='SENT').length,replies:a.filter(p=>p.status==='REPLIED').length,won:a.filter(p=>p.status==='WON / PAID').length,skipped:a.filter(p=>p.status==='SKIP').length,done:a.filter(processed).length,due:a.filter(p=>p.status==='SENT'&&isFollowDue(p)).length}}
function updateHeader(){const c=counters();$('doneCount').textContent=c.done;$('sentCount').textContent=c.sent;$('replyCount').textContent=c.replies;$('wonCount').textContent=c.won;if($('dueCount'))$('dueCount').textContent=c.due;if($('followBadge')){$('followBadge').textContent=c.due||'';$('followBadge').classList.toggle('show',c.due>0)}$('progressBar').style.width=Math.min(100,c.done/200*100)+'%';}
